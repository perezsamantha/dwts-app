...
import { Container, TeamName, CardContainer, CardAvatar, Header, LikesContainer } from '../shared/shared.js'
...
function TeamCard(props) {
    ...
    // separate loadings?
    const loading = useSelector(state => state.loading.TEAMFIND || state.loading.TEAMUPDATE);
        ...
        (loading ? <CircularProgress className={classes.progress} /> :
            <CardContainer>
                <Header>
                    ...
                    <LikesContainer>
                        <Button disableRipple onClick={() => dispatch(likeTeam(id))}>
                            <Likes />
                        </Button>
                ...
                <TeamName>{celeb?.first_name} {celeb?.last_name} & {pro?.first_name} {pro?.last_name}</TeamName>
                ...
                <Grid container justifyContent="center" className={classes.root} spacing={2}>
                    <Grid item>
                        ...
                        <SocialsRow>
                            {celeb?.instagram && <SocialsText href={'https://www.instagram.com/' + celeb.instagram}>@{celeb.instagram}</SocialsText>}
                            {pro?.instagram && <SocialsText href={'https://www.instagram.com/' + pro.instagram}>@{pro.instagram}</SocialsText>}
                        </SocialsRow>
                    </Grid>
                    <Grid item>
                        <TwitterIcon className={classes.icons} />
                        <SocialsRow>
                            {celeb?.twitter && <SocialsText href={'https://www.twitter.com/' + celeb.twitter}>@{celeb.twitter}</SocialsText>}
                            {pro?.twitter && <SocialsText href={'https://www.twitter.com/' + pro.twitter}>@{pro.twitter}</SocialsText>}
                        </SocialsRow>
                    </Grid>
                    <Grid item>
                        <FacebookIcon className={classes.icons} />
                        <SocialsRow>
                            {celeb?.tiktok && <SocialsText href={'https://www.tiktok.com/' + celeb.tiktok}>@{celeb.tiktok}</SocialsText>}
                            {pro?.tiktok && <SocialsText href={'https://www.tiktok.com/' + pro.tiktok}>@{pro.tiktok}</SocialsText>}
                        </SocialsRow>
                    </Grid>
                </Grid>
