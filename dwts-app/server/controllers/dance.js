...
export const findDanceById = async (req, res) => {
    ...
    try {
        const dance = await pool.query('SELECT * FROM dances WHERE dance_id = $1', [id]);
...
export const updateDance = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('UPDATE dances SET style = $1, episode_id = $2, theme = $3, running_order = $4, song_title = $5, song_artist = $6, link = $7, extra = $8 WHERE dance_id = $9 RETURNING *', [style, episode_id, theme, running_order, song_title, song_artist, link, extra, id]);
...
export const deleteDance = async (req, res) => {
    ...
    try {
        await pool.query('DELETE FROM dances WHERE dance_id = $1', [id]);
...
export const addPic = async (req, res) => {
    ...
    try {
        ...
        blobWriter.on('finish', async () => {
            ...
            const result = await pool.query('UPDATE dances SET pictures = array_append(pictures, $1) WHERE dance_id = $2 RETURNING *', [publicUrl, req.params.id]);
...
export const likeDance = async (req, res, next) => {
    try {
        ...
        if (pool.query(`exists(SELECT 1 FROM dance_likes WHERE dance_id = ${id}, user_id = ${userId}`)) {
            await pool.query(`DELETE FROM dance_likes WHERE dance_id = ${id}, user_id = ${userId}`);
            // json message
        } else {
            const result = await pool.query(`INSERT INTO dances (dance_id, user_id) VALUES($1, $2)`, [id, userId]);
